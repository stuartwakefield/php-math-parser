...
The next pattern becomes more complicated, `S -> S add S`. The pattern is a mix of terminals,
symbols which there are no subsequent transformation rules and and non-terminals. The algorithm should 
leverage the fact that the `S` symbol has to be a chain of tokens where the minimum length of the chain is 
1 and there is no maximum length and that the `add` symbol is the least expensive symbol to check. 

It is logical to check the tokens from index 2 to the second to last index against a match for the 
`add` pattern. In this case the first token it checks is a match. It will now verify `S add S`, splitting
the tokens for the first S to `integer` and the last S as `integer divide integer`. 

Running through the rules again from the top on the first `S` we get a match at `S -> integer`. 
For the last `S`, we again run through the rules from the top, we get to the first rule with non-terminals 
and check becomes much more simple as there are only three tokens left in this expression and the pattern 
is a minimum of three tokens the algorithm only needs to check the terminal symbol against one token 
`divide` ignoring the `integer` tokens on either side. `S add S` is not a match, it gets to `S divide S` 
and it is a match. It has now resolved `integer add S divide S`. The left hand `S` of the expression is 
the token `integer`, which is resolved to `S -> integer`, the right hand `S` of the expression is also 
`integer` which is also resolved to `S -> integer`.
